...
export function generateAndDownloadSitrep(options: SitrepExportOptions = {}): string {
  ...
  const cards: Array<{ label: string; val: string; color: RGBColor; tag: string }> = [
    ...
    { label: "High Rescues", val: String(highPriorityRescues), color: [245, 158, 11], tag: "EN ROUTE" },
    { label: "Resolved Cases", val: String(resolvedCases), color: [16, 185, 129], tag: "TODAY" },
    { label: "Data Sources", val: String(connectedSources), color: [59, 130, 246], tag: "CONNECTED" },
  ];
  ...
  const units = [
    { id: "UNIT-12", type: "NDRF Swift Water", crew: "6 Pers", loc: "Sabarmati Riverfront", status: "EN ROUTE (ETA 4 min)" },
    { id: "UNIT-07", type: "AMC Road Clearance", crew: "4 Pers", loc: "SG Highway / ISCON", status: "ON SCENE" },
    { id: "UNIT-21", type: "Medical ALS Unit", crew: "8 Pers", loc: "Maninagar Station", status: "STANDBY" },
    { id: "UNIT-33", type: "Structural Rescue", crew: "5 Pers", loc: "Satellite Sector", status: "RETURNING" },
  ];
  ...
  doc.setTextColor(100, 116, 139);
  doc.text(
    "Automated SITREP generated by RescuAI Disaster Operations Console · Gujarat State Disaster Management Authority (GSDMA).",
    margin,
    280
  );
  doc.text(
